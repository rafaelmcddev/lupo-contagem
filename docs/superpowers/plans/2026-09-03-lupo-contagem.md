...
- Every exact barcode is mandatorily linked to a SKU (a string), keyed on the exact barcode (not a prefix — unlike groups). A scan is never recorded until its barcode already has, or is given, a SKU: if the barcode has no registered SKU, the scan endpoint rejects with `sku_required` and records nothing; the client then prompts for a SKU and resubmits the same scan with it. Once linked, all future scans of that exact barcode carry the SKU automatically. This invariant (every recorded `scans` row has a matching `skus` row) means SKU breakdowns are a plain inner join, never a nullable lookup.
- A box's per-SKU breakdown is part of its display and its exports, alongside the box number and group name — one box commonly contains several SKUs (e.g. different sizes/colors sharing a group prefix).
...
lupo-contagem/
  ...
  .env.example
  db/
    schema.ts                      -- all 6 Drizzle tables
...
**Interfaces:**
- Produces: `countings`, `boxes`, `scans`, `settings`, `groups`, `skus` Drizzle tables from `db/schema.ts`, and `resetDb()` from `tests/resetDb.ts` — both used by every integration test from Task 9 onward.
...
Expected: a new folder appears under `db/migrations/` with SQL creating the 6 tables and the enum; "Migrations applied" is printed.
...
```bash
git add -A
git commit -m "feat: database schema and migrations for countings, boxes, scans, settings, groups, skus"
```
...
**Interfaces:**
- Produces: `interface SkuBreakdownEntry { sku: string; total: number }`, `interface BoxSummary { boxNumber: number; groupName: string | null; total: number; skuBreakdown: SkuBreakdownEntry[] }`, `interface CountingInfo { name: string }`, `toCsv(counting: CountingInfo, boxes: BoxSummary[]): string`, `toWhatsAppText(counting: CountingInfo, boxes: BoxSummary[]): string` — used by `components/ExportButtons.tsx` (Task 25).
...
### Task 11: Core scan logic — box assignment, dedupe, group resolution, SKU linking
...
**Interfaces:**
- Consumes: `db`, `DbClient` (Task 2); `countings`, `boxes`, `scans`, `groups`, `skus` tables (Task 3); `extractPrefix`, `isValidBarcode` (Task 4); `isDuplicateScan` (Task 5); `resolveGroupName` (Task 6); `resetDb` (Task 3).
- Produces: `class InvalidBarcodeError`, `class CountingNotFoundError`, `class CountingNotActiveError`, `class SkuRequiredError`, `interface BoxHit { boxId: number; boxNumber: number; prefix: string; groupName: string | null; sku: string; total: number }`, `interface ScanOutcome { duplicate: boolean; box: BoxHit | null }`, `recordScan(db: DbClient, countingId: number, rawBarcode: string, sku?: string): Promise<ScanOutcome>` — used by `app/api/countings/[id]/scan/route.ts` (Task 14). A scan is never recorded until its exact barcode has a `skus` row: if none exists and no `sku` argument is given, `recordScan` throws `SkuRequiredError` and writes nothing.
...
```bash
git add -A
git commit -m "feat: core scan logic with atomic box assignment, dedupe, and mandatory SKU linking"
```
...
- Produces: `GET /api/countings/:id` returning `{ counting, boxes: BoxSummary[], grandTotal }` where each `BoxSummary` includes `skuBreakdown: { sku: string; total: number }[]` (Task 7's shape) — used by `app/countings/[id]/page.tsx` (Task 25).
...
```bash
git add -A
git commit -m "feat: counting detail API with per-box totals, group names, and SKU breakdown"
```
...
**Interfaces:**
- Consumes: `db` (Task 2); `countings` table (Task 3); `recordScan`, `InvalidBarcodeError`, `CountingNotFoundError`, `CountingNotActiveError`, `SkuRequiredError` (Task 11).
- Produces: `POST /api/countings/:id/scan {barcode, sku?}` returning `ScanOutcome` JSON, or a 422 `{ error: 'sku_required' }` when the exact barcode has no linked SKU and none was provided — used by `app/countings/[id]/page.tsx` (Task 25).
...
```bash
git add -A
git commit -m "feat: scan API endpoint with mandatory SKU linking"
```
...
- Produces: `interface SkuBreakdownEntry { sku: string; total: number }`, `interface BoxSummary { boxNumber: number; groupName: string | null; total: number; skuBreakdown: SkuBreakdownEntry[] }`, `<BoxList boxes: BoxSummary[] />` — used by `app/countings/[id]/page.tsx` (Task 25).
...
```bash
git add -A
git commit -m "feat: large-text live box list with per-SKU breakdown"
```
...
**Interfaces:**
- Consumes: `GET /api/countings/:id`, `POST /api/countings/:id/scan` (now `{barcode, sku?}`, may respond 422 `sku_required`), `POST /api/countings/:id/finish` (Tasks 13-15); `BarcodeInput` (Task 17, its `disabled` prop); `CameraScanner` (Task 19); `BoxList` (Task 18); `ExportButtons` (Task 20); `useSpeechAnnouncer` (Task 16); `Button`, `PageHeading` (Task 8).

This page branches on `counting.status`: while `active` it shows the scanner input, camera button, live box list, and "Finalizar contagem"; once `finished` it shows the same box list plus `ExportButtons` instead of the scanner controls. It polls the detail endpoint every 3 seconds while active so scans from other people/devices in the same counting show up. When a scan comes back with `sku_required`, the barcode input is disabled and a form asking for the SKU appears; submitting it resubmits the same scan with the SKU attached.
...
```bash
git add -A
git commit -m "feat: Contagem ativa / Resumo page wiring scanner, camera, audio, SKU prompt, live list, export"
```
...
**Interfaces:**
- Produces: `interface QueuedScan { countingId: string; barcode: string; sku?: string; queuedAt: number }`, `loadQueue(): QueuedScan[]`, `saveQueue(queue: QueuedScan[]): void`, `enqueueScan(countingId: string, barcode: string, sku?: string): QueuedScan[]`, `removeFromQueue(queue: QueuedScan[], toRemove: QueuedScan): QueuedScan[]` — used by `app/countings/[id]/page.tsx`. `removeFromQueue` matches by `queuedAt`, not object identity, because the page calls `loadQueue()` fresh at several points and each call returns newly-parsed objects that are never reference-equal to an earlier read.
...
Replace the file with this full content (keeps Task 25's four tests unchanged, adds one at the end):
...
This merges Task 25's SKU-prompt flow with the retry queue: `submitScan` returns `{status, data}` so both the interactive path and the queue-flush path can detect `sku_required`; if a *queued* retry comes back needing a SKU (only possible for an item that was queued before its 422 was ever seen), the flush stops and hands that barcode to the same interactive SKU form instead of silently dropping it.
...
```bash
git add -A
git commit -m "feat: client-side retry queue for scans lost to network failures, SKU-aware"
```
...
- [ ] **Step 2: Walk the golden path in a browser**
  ...
  - In the barcode input, type a 7+ digit numeric code and press Enter — confirm the barcode input becomes disabled and a form appears asking for the SKU of that code. Type a SKU and submit — confirm a "Caixa 1" card appears showing that SKU and its count, the total increments, and the browser speaks "Caixa 1".
  - Type the same code again immediately — confirm it is ignored (no count change, no SKU prompt).
  - Type a different code sharing the same first digits (same group) but never scanned before — confirm it's prompted for its own SKU, and once submitted it lands in "Caixa 1" alongside the first SKU, with both SKUs and their individual counts visible in the box's breakdown.
  - Type a code with a different prefix (with its own new SKU) — confirm "Caixa 2" appears and is announced.
  ...
  - Click "Finalizar contagem" — confirm it moves to the summary view with export buttons, that the SKU breakdown is visible per box, and that "Exportar CSV" downloads a file with a SKU column and "Copiar texto WhatsApp" copies text with an indented line per SKU.
  ...
  - Open Grupos, register a prefix/name matching a code used above, revisit that counting's summary (or history) — confirm the group name now shows next to the box number (SKU breakdown still shows underneath).
  - With devtools open, switch the Network tab to "Offline", scan an already-known code — confirm a "Sincronizando leituras pendentes..." message appears instead of the scan being silently lost; switch back to "Online" — confirm the message disappears and the box list updates with that scan counted.
